...
export default function Header() {
    ...
    return (
        <header className="">
            <div className={"bg-custom-1 flex flex-col md:flex-row justify-end text-white text-md md:text-2xl py-3 px-5 md:space-x-2 space-x-0"}>
            ...
            <div className={"border-y-2 border-custom-3 py-2"}>
                <div className={"bg-custom-3 text-xl gap-2 py-2 pr-2 flex md:flex-col flex-row justify-between"}>
                    <div className="flex md:flex-row flex-col justify-evenly" >
                        {navBarCategories.map(item => (
                            <Link key={item.name} href={item.path}>
                                <div className="cursor-pointer hover:shadow-lg py-1 px-3 rounded-lg font-bold" hidden={(windowWidth < 768) && hideMenu}>
                                    {item.name}
                                </div>
                            </Link>
                        ))}
                    ...
                    <div hidden={windowWidth > 768}>
                        <button onClick={()=>{setHideMenu(!hideMenu)}} className="hover:shadow-md px-3 py-0.5 rounded-lg">
                            Menu
                        </button>
                    </div>
                </div>
